...
class WebcamApp:
    def __init__(self, root):
        ...
        self.script1_thread = None
        self.script1_complete = False
    ...
    def execute_script1(self):
        if not self.script1_running:
            ...
            self.script1_thread = Thread(target=self.execute_script, args=(script_path,))
            self.script1_thread.start()

            # Start checking for thread completion
            self.root.after(100, self.check_script1_completion)
    ...
    def execute_script(self, script_path):
        ...
        self.script1_complete = True

    def check_script1_completion(self):
        if self.script1_complete:
            self.script1_thread.join()
            self.script1_thread = None
            self.close_app()
        else:
            self.root.after(100, self.check_script1_completion)
